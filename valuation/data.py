"""yfinance 기반 입력 데이터 수집 파이프라인.

보고서의 데이터 출처 원칙을 따른다:
- 무료/저비용 출처(야후 파이낸스) 우선, 원천 데이터는 SEC EDGAR 공시 기반
- 주식수는 단순 상장 주식수가 아닌 완전 희석 주식수(Diluted Shares) 우선 적용
...
def fetch_inputs(ticker: str) -> CompanyInputs:
    """단일 종목의 밸류에이션 입력 데이터를 수집한다.

    ETF는 자체 현금흐름이 없어 본 템플릿의 평가 대상이 아니므로 거부한다.
    """
    t = yf.Ticker(ticker)
    info: dict = {}
    try:
...
def fetch_historical_inputs(ticker: str) -> list[CompanyInputs]:
    """연간 재무제표(최대 4개년)로 과거 회계연도별 입력 데이터를 재구성한다.

    적정주가 궤적 오버레이 차트에서 내재가치가 시장 가격에 선행했는지
    복기하는 용도. 과거 시점의 성장률 추정치는 구할 수 없으므로
    해당 연도 실적 기반의 보수적 추정에 그친다.
    """
    t = yf.Ticker(ticker)
    income = _safe_statement(t, "income_stmt")
    balance = _safe_statement(t, "balance_sheet")
