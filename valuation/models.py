...
def dcf_explain(inputs: CompanyInputs, a: Assumptions) -> dict[str, Any]:
    ...
    for t in range(1, n + 1):
        # 2단계 성장: 초기 성장률 g를 영구 성장률 g∞로 선형 감쇠(fade).
    ...
    steps.append(_step(
        f"추정기간({n}년) FCF 현재가치 합 ΣPV (2단계 성장 fade={a.dcf_fade})",
        "g_t = g + (g∞−g)×(t−1)/(n−1),  PV_t = FCFₜ/(1+r)^t,  ΣPV = Σ PV_t",
        f"FCF₀={_n(inputs.fcf)}, g={_n(growth)}→g∞={_n(a.terminal_growth)}, "
        f"r={_n(r)} → 연도별 현가 합산",
        pv,
