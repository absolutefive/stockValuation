...
| **적용한계 플래그** (out_of_domain) | 밴드 괴리율이 ±300%를 넘고 동시에 모델이 크게 발산(CV ≥ 0.35)하면 점추정 적정가의 정밀도가 무의미해진다. 이때 `과열 경고` 대신 **`측정한계`** 신호로 분리해 거짓 정밀도(예: 3020% 괴리율)를 격리한다. 적정가 계산식은 건드리지 않고, 모델이 *자기 유효범위 밖*임을 스스로 알린다 |
...
| 측정한계 | 🟣 측정한계 | 밴드 괴리 ±300% 초과 + 모델 발산(CV ≥ 0.35) 동시 성립. 펀더멘털로 설명 불가능한 내러티브 프리미엄 구간으로, 괴리율 **숫자 자체는 무의미**하고 방향·추세로만 해석. 보고서엔 `범위 초과`로 표기 (예: TSLA) |
